...
module.exports = class eWeLinkWS {
   constructor(log, apiHost, aToken, apiKey, debug) {
      platform = this;
      platform.log = log;
   ...
   login() {
      ...
      platform.ws.onopen = function (e) {
         ...
         if (platform.debug) {
            let rct = JSON.stringify(payload, null, 2).replace(platform.aToken, "**hidden**").replace(platform.apiKey, "**hidden**");
      ...
      platform.ws.onmessage = function (m) {
         ...
         if (platform.debug) {
            let rct = JSON.stringify(device, null, 2).replace(device.deviceid, "***hidden***").replace(device.apikey, "***hidden***");
   ...
   sendUpdate(json, callback) {
      ...
      let sendOperation = (string) => {
         ...
         if (platform.ws) {
            ...
            if (platform.debug) {
               let rct = JSON.stringify(json, null, 2).replace(json.apikey, "**hidden**").replace(json.deviceid, "**hidden**");
