...
module.exports = class eWeLinkHTTP {
   ...
   login() {
      ...
      if (this.debug) {
         let rct = JSON.stringify(data, null, 2).replace(this.config.username, "**hidden**").replace(this.config.password, "**hidden**");
      ...
      return new Promise((resolve, reject) => {
         axios({
            ...
            if (body.hasOwnProperty('error') && body.error == 301 && body.hasOwnProperty('region')) {
               let idx = this.httpHost.indexOf('-');
               if (idx == -1) {
                  throw "Received new region [" + body.region + "]. However we cannot construct the new API host url.";
               }
               let newApiHost = body.region + this.httpHost.substring(idx);
               if (this.httpHost != newApiHost) {
                  if (this.debug) this.log("Received new region [%s], updating API host to [%s].", body.region, newApiHost);
                  this.httpHost = newApiHost;
                  this.login();
                  return;
               }
            }
