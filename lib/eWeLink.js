...
class eWeLink {
      ...
      try {
         ...
         case "valve":
            accessory.addService(Service.Valve, "Valve A", "valvea")
               ...
               .setCharacteristic(Characteristic.SetDuration, 120)
               .setCharacteristic(Characteristic.RemainingDuration, 120);
   ...
   configureAccessory(accessory) {
      ...
      try {
         ...
         case "valve":
            ...
            accessory.getService("Valve A").getCharacteristic(Characteristic.SetDuration)
               .on("set", (value, callback) => {
                  if (accessory.getService("Valve A").getCharacteristic(Characteristic.InUse).value) {
                     accessory.getService("Valve A").updateCharacteristic(Characteristic.RemainingDuration, value);
                     clearTimeout(accessory.getService("Valve A").timer);
                     accessory.getService("Valve A").timer = setTimeout(() => {
                        accessory.getService("Valve A").setCharacteristic(Characteristic.Active, 0);
                     }, (value * 1000));
                  }
                  callback();
               });
            ...
            accessory.getService("Valve B").getCharacteristic(Characteristic.SetDuration)
               .on("set", (value, callback) => {
                  if (accessory.getService("Valve B").getCharacteristic(Characteristic.InUse).value) {
                     accessory.getService("Valve B").updateCharacteristic(Characteristic.RemainingDuration, value);
                     clearTimeout(accessory.getService("Valve B").timer);
                     accessory.getService("Valve B").timer = setTimeout(() => {
                        accessory.getService("Valve B").setCharacteristic(Characteristic.Active, 0);
                     }, (value * 1000));
                  }
                  callback();
               });
   ...
   externalValveUpdate(accessory, params) {
      try {
         ...
         if (params.switches[0].switch === "on") {
            let timer = accessory.getService("Valve A").getCharacteristic(Characteristic.SetDuration).value;
            accessory.getService("Valve A").updateCharacteristic(Characteristic.RemainingDuration, timer);
            accessory.getService("Valve A").timer = setTimeout(() => {
               accessory.getService("Valve A").setCharacteristic(Characteristic.Active, 0);
            }, (timer * 1000));
         } else {
            accessory.getService("Valve A").updateCharacteristic(Characteristic.RemainingDuration, 0);
            clearTimeout(accessory.getService("Valve A").timer);
         }
         if (params.switches[1].switch === "on") {
            let timer = accessory.getService("Valve B").getCharacteristic(Characteristic.SetDuration).value;
            accessory.getService("Valve B").updateCharacteristic(Characteristic.RemainingDuration, timer);
            accessory.getService("Valve B").timer = setTimeout(() => {
               accessory.getService("Valve B").setCharacteristic(Characteristic.Active, 0);
            }, (timer * 1000));
         } else {
            accessory.getService("Valve B").updateCharacteristic(Characteristic.RemainingDuration, 0);
            clearTimeout(accessory.getService("Valve B").timer);
         }
      } catch (err) {
