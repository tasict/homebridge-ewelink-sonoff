...
class eWeLink {
   ...
   addAccessory(device, hbDeviceId, service) {
      ...
      try {
         ...
         case "cusBlind":
            ...
            let group = platform.customGroups.get(accessory.context.hbDeviceId.toUpperCase());
            accessory.context = {
               ...accessory.context,
               ...{
                  operationTime: group.operationTime || 10,
                  operationTimePercent: (group.operationTime || 10) * 10,
                  cachePosition: 100 // default to fully open on device add
               }
            };
            break;
         case "cusGarage":
         ...
         case "bridge":
            ...
            if (platform.customBridgeSensors.has(hbDeviceId.toUpperCase())) {
   ...
   internalBlindUpdate(accessory, value, callback) {
      ...
      let payload = {
         ...
         params: constants.defaultParamsBlind
      };
      if (cSta < 2) { // ie it's currently moving [either up or down]
         ...
         if (diffPosition > 0) {
            ...
            if (diff < 0) {
               ...
               payload.params.switches[0].switch = cSta === 1 ? "on" : "off";
               payload.params.switches[1].switch = cSta === 0 ? "on" : "off";
               platform.api.updatePlatformAccessories("homebridge-ewelink-sonoff", "eWeLink", [accessory]);
      ...
      platform.api.updatePlatformAccessories("homebridge-ewelink-sonoff", "eWeLink", [accessory]);
      payload.params.switches[0].switch = moveUp ? "on" : "off";
      payload.params.switches[1].switch = moveUp ? "off" : "on";
      platform.wsClient.sendUpdate(payload, function () {
         return;
      });
   ...
   externalBlindUpdate(accessory, params) {
      ...
      let switchUp = params.switches[0].switch === "on" ? -1 : 0; // matrix of numbers to get correct HomeKit value
      let switchDown = params.switches[1].switch === "on" ? 0 : 2;
      let state = [0, 1, 2].includes(switchUp + switchDown) ? switchUp + switchDown : 2; // ie if sum is -1
      let tPos = state === 2 ? accessory.context.cachePosition : state * 100;
      accessory.getService(Service.WindowCovering)
      ...
      try {
         accessory.context.cachePosition = tPos;
         platform.api.updatePlatformAccessories("homebridge-ewelink-sonoff", "eWeLink", [accessory]);
      } catch (e) {
