...
class eWeLink {
   ...
   internalBlindUpdate(accessory, value, callback) {
      ...
      if (!platform.customGroups.has(accessory.context.hbDeviceId.toUpperCase()) || platform.customGroups.get(accessory.context.hbDeviceId.toUpperCase()).type !== "cusBlind") {
         callback("[" + accessory.displayName + "] could not be updated as group config missing.");
         return;
      }
      ...
      if (!["oneSwitch", "twoSwitch"].includes(blindConfig.setup)) {
         callback("[" + accessory.displayName + "] could not be updated due to improper configuration.");
      }
      let sensorAccessory = false;
      if (!!blindConfig.sensorFullId && !platform.devicesInHB.has(blindConfig.sensorFullId.toUpperCase())) {
         callback("[" + accessory.displayName + "] could not be updated as defined sensor doesn't exist.");
      } else {
      ...
      if (blindConfig.setup === "oneSwitch" && !sensorAccessory) {
         callback("[" + accessory.displayName + "] could not be updated as type [oneSwitch] requires a sensor.");
         return;
      }
      let cPos;
      if (sensorAccessory) {
      ...
      if (cPos === value) {
         ...
         callback(); // true callback even though nothing to do.
         return;
      }
