...
class eWeLink {
   ...
   initialiseDevice(device) {
      ...
      if (!this.refreshAccessory(accessory, device.params)) {
         this.log.error("[%s] cannot be refreshed due to missing type parameter. Please remove accessory from the Homebridge cache along with any secondary devices (SW1, SW2, etc.). Debugging: [%s:%s:%s]", accessory.displayName, "initialise", accessory.context.type, accessory.context.channelCount);
      }
   }
