...
class eWeLink {
         ...
         this.httpClient.getHost()
            .then(res => { //*** Use the HTTP API host to log into eWeLink ***\\
               ...
               (() => {
                  ...
                  //*** Make a map of custom devices from Homebridge config ***\\
                  if (this.config.customDevices && Object.keys(this.config.customDevices).length > 0) {
                     this.config.customDevices.forEach(device => {
                        if (typeof device.deviceId !== "undefined" && this.devicesInEwe.has(device.deviceId.toLowerCase())) {
                           this.customDevices.set(device.deviceId, device);
                        }
                     });
                  }
   ...
   initialiseDevice(device) {
      ...
      if (!this.devicesInHB.has(device.deviceid + "SWX") && !this.devicesInHB.has(device.deviceid + "SW0")) {
         if (this.customDevices.has(device.deviceid) && this.customDevices.get(device.deviceid).type === "valve") {
            for (let i = 0; i <= 2; i++) {
               this.addAccessory(device, device.deviceid + "SW" + i, "valve");
            }
         } else if (this.customGroups.has(device.deviceid + "SWX") && this.customGroups.get(device.deviceid + "SWX").type === "blind") {
   ...
   addAccessory(device, hbDeviceId, service) {
      ...
      try {
         ...
         switch (service) {
         case "valve":
            accessory.addService(Service.Valve)
   ...
   configureAccessory(accessory) {
      ...
      try {
         ...
         case "valve":
            accessory.getService(Service.Valve).getCharacteristic(Characteristic.Active)
               .on("set", (value, callback) => {
                  accessory.getService(Service.Valve).getCharacteristic(Characteristic.Active).value !== value ?
                     this.internalValveUpdate(accessory, value, callback) :
                     callback();
               });
            break;
         case "blind":
   ...
   refreshAccessory(accessory, newParams) {
      ...
      case "valve":
         if (Array.isArray(newParams.switches)) {
            this.externalMultiValveUpdate(accessory, newParams);
            return true;
         }
         break;
      case "blind":
      ...
      this.lanClient.sendUpdate(payload)
         ...
         .catch(err => {
            if (accessory.context.reachableWAN) {
               if (this.debug) {
                  this.log.warn("[%s] LAN update failed [%s]. Trying web socket update.", accessory.displayName, err);
               }
   ...
   internalValveUpdate(accessory, value, callback) {
      try {
         if (!accessory.context.reachableWAN && !accessory.context.reachableLAN) {
         ...
         case "0":
            ...
            accessory.getService(Service.Valve).updateCharacteristic(Characteristic.Active, value);
            accessory.getService(Service.Valve).updateCharacteristic(Characteristic.InUse, value);
            for (let i = 1; i <= 4; i++) {
               if (this.devicesInHB.has(accessory.context.eweDeviceId + "SW" + i)) {
                  ...
                  oAccessory.getService(Service.Valve).updateCharacteristic(Characteristic.Active, value);
                  oAccessory.getService(Service.Valve).updateCharacteristic(Characteristic.InUse, value);
               }
            }
            break;
         case "1":
         case "2":
            ...
            for (let i = 1; i <= 4; i++) {
               if ((tAccessory = this.devicesInHB.get(accessory.context.eweDeviceId + "SW" + i))) {
                  i === parseInt(accessory.context.switchNumber) ?
                     ...
                     payload.params.switches[i - 1].switch = tAccessory.getService(Service.Valve).getCharacteristic(Characteristic.Active).value ? "on" : "off";
                  tAccessory.getService(Service.Valve).getCharacteristic(Characteristic.InUse).value ? "on" : "off";
                  if (tAccessory.getService(Service.Valve).getCharacteristic(Characteristic.Active).value) {
                     masterState = "on";
                  }
               } else {
            ...
            oAccessory.getService(Service.Valve).updateCharacteristic(Characteristic.Active, masterState === "on");
            oAccessory.getService(Service.Valve).updateCharacteristic(Characteristic.InUse, masterState === "on");
            break;
         default:
   ...
   internalGarageUpdate(accessory, value, callback) {
      try {
         ...
         if (garageConfig.setup === "twoSwitch") {
            ...
            let sensorState = sAccessory.getService(Service.ContactSensor).getCharacteristic(Characteristic.ContactSensorState).value;
            //sensorState = 0 -> contact detected -> garage closed // 1 -> contact not detected -> garage open
            this.log("[%s] has received request to [%s].", value ? "close" : "open");
            this.log("[%s] reports that [%s] is [%s].", sAccessory.displayName, accessory.displayName, sensorState ? "open" : "closed");
            if (value !== sensorState) {
               // a mismatch has happened eg door has been requested to open when it is already open.
               // we should just update the revised status of the door based on the sensor
               accessory.getService(Service.GarageDoorOpener)
                  .updateCharacteristic(Characteristic.TargetDoorState, sensorState ? 0 : 1)
                  .updateCharacteristic(Characteristic.CurrentDoorState, sensorState ? 0 : 1);
               callback();
               return;
            }
         }
         ...
         accessory.getService(Service.GarageDoorOpener)
            .updateCharacteristic(Characteristic.TargetDoorState, value ? 0 : 1)
            .updateCharacteristic(Characteristic.CurrentDoorState, value ? 2 : 3);
         ...
         if (!sAccessory) {
            setTimeout(() => {
               accessory.getService(Service.GarageDoorOpener)
                  .updateCharacteristic(Characteristic.CurrentDoorState, value ? 0 : 1);
               callback();
   ...
   internalUSBUpdate(accessory, value, callback) {
      try {
         ...
         let payload = {
            apikey: accessory.context.eweApiKey,
            deviceid: accessory.context.eweDeviceId,
            params: this.devicesInEwe.get(accessory.context.eweDeviceId).params.switches
         };
         payload.params.switches[0].switch = value ? "on" : "off";
         if (this.debug) {
            this.log("[%s] requesting to turn [%s].", accessory.displayName, value ? "on" : "off");
   ...
   externalMultiValveUpdate(accessory, params) {
      try {
         ...
         for (let i = 1; i <= accessory.context.channelCount; i++) {
            if (this.devicesInHB.has(idToCheck + i)) {
               ...
               oAccessory.getService(Service.Valve).updateCharacteristic(Characteristic.Active, params.switches[i - 1].switch === "on");
               oAccessory.getService(Service.Valve).updateCharacteristic(Characteristic.InUse, params.switches[i - 1].switch === "on");
               if (params.switches[i - 1].switch === "on") {
                  primaryState = true;
               }
            }
         }
         accessory.getService(Service.Valve).updateCharacteristic(Characteristic.Active, primaryState);
         accessory.getService(Service.Valve).updateCharacteristic(Characteristic.InUse, primaryState);
      } catch (err) {
   ...
   externalSensorUpdate(accessory, params) {
      try {
         ...
         // VALUES FOR OLDSTATE AND NEWSTATE ARE:
         //   0 -> contact detected -> garage closed
         //   1 -> contact not dtct -> garage open
         ...
         this.customGroups.forEach(group => {
            if (group.sensorId === accessory.context.eweDeviceId && group.type === "garage") {
               if ((oAccessory = this.devicesInHB.get(group.deviceId + "SWX"))) {
                  if (newState === 0) { // the garage must now be FULLY closed so lets update it immediately
                  ...
                  } else if (newState === 1) { // sensor broken so garage must have STARTED to open.
                     setTimeout(() => {
                        oAccessory.getService(Service.GarageDoorOpener)
