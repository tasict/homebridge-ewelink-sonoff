   ...
   receiveDeviceUpdate(device) {
      ...
      case "update":
         if (this.devicesInHB.has(device.deviceid + "SWX") || this.devicesInHB.has(device.deviceid + "SW0")) {
            ...
            if (this.debug) {
               this.log("[%s] was externally updated (see above WS/LAN message) and will be refreshed.", accessory.displayName);
            }
            if (this.refreshAccessory(accessory, device.params)) {
               return;
            }
            this.log.error("[%s] cannot be refreshed due to missing type parameter. Please remove accessory from the Homebridge cache along with any secondary devices (SW1, SW2, etc.).", accessory.displayName);
