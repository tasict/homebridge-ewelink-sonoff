...
class eWeLink {
   ...
   addAccessory(device, hbDeviceId, service) {
      ...
      try {
         ...
         case "bridge":
            let b;
            if ((b = accessory.context.sensorType = platform.customBridgeSensors.get(hbDeviceId.toUpperCase()))) {
   ...
   removeAllAccessories() {
      platform.log.warn("[0] primary devices were loaded from your eWeLink account so any eWeLink devices in the Homebridge cache will be removed. This plugin will no longer load as there is no reason to continue.");
      try {
