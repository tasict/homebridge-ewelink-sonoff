...
class eWeLink {
                  ...
                  if (this.config.customDevices && Object.keys(this.config.customDevices).length > 0) {
                     this.config.customDevices.forEach(device => {
                        if (typeof device.deviceId !== "undefined" && this.devicesInEwe.has(device.deviceId.toLowerCase())) {
                           this.cusD.set(device.deviceId, device);
                        }
                     });
                  }
   ...
   initialiseDevice(device) {
      ...
      if (!this.devicesInHB.has(device.deviceid + "SWX") && !this.devicesInHB.has(device.deviceid + "SW0")) {
         if (this.cusD.has(device.deviceid) && this.cusD.get(device.deviceid).type === "valve") {
            switch (constants.chansFromUiid[device.uiid]) {
            case 1:
      ...
      try {
         ...
         case "bridge":
            accessory.context.sensorType = this.cusS.has(hbDeviceId) ?
               this.cusS.get(hbDeviceId).type : "motion";
   ...
   refreshAccessory(accessory, newParams) {
      ...
      case "valve":
         if (newParams.hasOwnProperty("switch")) {
            this.externalSingleValveUpdate(accessory, newParams);
            return true;
         } else if (Array.isArray(newParams.switches)) {
   ...
   internalValveUpdate(accessory, value, callback) {
      try {
         ...
         case "0":
            ...
            for (let i = 1; i <= accessory.context.channelCount; i++) {
               if (this.devicesInHB.has(accessory.context.eweDeviceId + "SW" + i)) {
                  ...
                  oAccessory.getService(Service.Valve)
                     .updateCharacteristic(Characteristic.Active, value)
                     .updateCharacteristic(Characteristic.InUse, value);
               }
            }
         ...
         case "2":
            ...
            for (let i = 1; i <= accessory.context.channelCount; i++) {
               if ((tAccessory = this.devicesInHB.get(accessory.context.eweDeviceId + "SW" + i))) {
                  i === parseInt(accessory.context.switchNumber) ?
                     ...
                     params.switches[i - 1].switch = tAccessory.getService(Service.Valve).getCharacteristic(Characteristic.Active).value ? "on" : "off";
            ...
            oAccessory.getService(Service.Valve)
               .updateCharacteristic(Characteristic.Active, masterState === "on")
               .updateCharacteristic(Characteristic.InUse, masterState === "on");
            break;
         default:
   ...
   externalSingleValveUpdate(accessory, params) {
      try {
         accessory.getService(Service.Valve)
            .updateCharacteristic(Characteristic.Active, params.switch === "on")
            .updateCharacteristic(Characteristic.InUse, params.switch === "on");
      } catch (err) {
      ...
      try {
         ...
         for (let i = 1; i <= 2; i++) {
            if (this.devicesInHB.has(idToCheck + i)) {
               ...
               oAccessory.getService(Service.Valve)
                  .updateCharacteristic(Characteristic.Active, params.switches[i - 1].switch === "on")
                  .updateCharacteristic(Characteristic.InUse, params.switches[i - 1].switch === "on");
         ...
         accessory.getService(Service.Valve)
            .updateCharacteristic(Characteristic.Active, primaryState)
            .updateCharacteristic(Characteristic.InUse, primaryState);
      } catch (err) {
         this.log.warn("[%s] could not be updated - [%s].", accessory.displayName, err);
      }
   }
