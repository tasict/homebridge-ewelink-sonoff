...
class eWeLink {
   ...
   initialiseDevice(device) {
      ...
      if ((accessory = this.devicesInHB.get(device.deviceid + "SWX"))) {
         ...
         if (accessory.context.eweUIID === 102) {
            accessory.context.reachableWAN = true; //DW2 on battery reports offline sometimes
         }
         if (this.debug) {
            let str = accessory.context.reachableLAN ?
               "and locally with IP " + accessory.context.reachableLAN :
               "but LAN mode unsupported";
            this.log("[%s] found in eWeLink %s. Refreshing.", accessory.displayName, str);
         }
         try {
      ...
      if (!this.refreshAccessory(accessory, device.params)) {
         this.log.error("[%s] could not be initialised due to missing type parameter.\nPlease remove device from the Homebridge cache, including any secondary devices (SW1, SW2, etc.).", accessory.displayName);
      }
   }
   addAccessory(device, hbDeviceId, service) {
      ...
      if (["1", "2", "3", "4"].includes(switchNumber)) {
         ...
         if (this.customHideChanFromHB.includes(hbDeviceId) && service === "switch") {
            this.log.warn("[%s] has not been added as per user configuration", newDeviceName);
            return;
         }
      }
      ...
      try {
         ...
         case "sensor":
            accessory.addService(Service.ContactSensor)
               .setCharacteristic(Characteristic.ContactSensorState, 0); // 0 = contact detected. default to closed.
            break;
         case "fan":
         ...
         case "bridge":
            accessory.context.sensorType = this.customBridgeSensors.has(hbDeviceId) ?
               this.customBridgeSensors.get(hbDeviceId).type :
               "motion";
   ...
   sendDeviceUpdate(accessory, payload, callback) {
      if (accessory.context.reachableLAN && this.mode === "lan") {
         // TODO. At the moment this will use web socket ie commented line below.
         // this.lanClient.sendUpdate(payload, callback);
   ...
   receiveDeviceUpdate(device) {
      ...
      if (device.action === "sysmsg") {
            ...
            for (let i = 0; i <= accessory.context.channelCount; i++) {
               try {
                  if (this.devicesInHB.has(device.deviceid + "SW" + i)) {
                     ...
                     this.log("[%s] has been reported [%s].", oAccessory.displayName, oAccessory.context.reachableWAN ? "online" : "offline");
                     this.devicesInHB.set(oAccessory.context.hbDeviceId, oAccessory);
                     this.api.updatePlatformAccessories("homebridge-ewelink-sonoff", "eWeLink", [oAccessory]);
                  }
               } catch (e) {
         ...
         if (this.devicesInHB.has(device.deviceid + "SWX") || this.devicesInHB.has(device.deviceid + "SW0")) {
            ...
            if (this.debug) {
               this.log("[%s] was externally updated and will be refreshed.", accessory.displayName);
            }
            if (this.refreshAccessory(accessory, device.params)) {
               return;
            }
            if (Object.keys(device.params).length === 0 || device.params.hasOwnProperty("power") || device.params.hasOwnProperty("rssi") || device.params.hasOwnProperty("uiActive") || device.params.hasOwnProperty("sledOnline")) {
               if (this.debug) {
                  this.log("[%s] ...the update is useless to Homebridge.", accessory.displayName);
               }
               return;
            } else {
               this.log.error("[%s] cannot be refreshed due to missing type parameter.\nPlease remove [%s] from the Homebridge cache (including any secondary devices (SW1, SW2, etc.).", accessory.displayName, accessory.displayName);
            }
         } else if (this.customHideDvceFromHB.includes(device.deviceid)) {
            this.log.warn("[%s] Accessory received via web socket/LAN update has been hidden from Homebridge as per user configuration.", device.deviceid);
         } else {
            this.log.warn("[%s] Accessory received via web socket/LAN update does not exist in Homebridge. If it's a new accessory please restart Homebridge so it is added.", device.deviceid);
         }
      } else {
         if (this.debug) {
            this.log.warn("[%s] unknown action or parameters received via web socket/LAN update.", device.name);
         }
      }
   }
   ...
   internalGarageUpdate(accessory, value, callback) {
      try {
         ...
         this.log("=== INTERNAL GARAGE UPDATE [IGU] ===");
         this.log("IGU1/6: user config looks good so start procedure...");
         ...
         this.log("IGU2/6: requesting to [%s] [%s]...", value === 0 ? "open" : "close", accessory.displayName);
         this.log("IGU3/6: send request to eWeLink to turn on switch...");
         this.sendDeviceUpdate(accessory, payload, function () {
            return;
         });
         this.log("IGU4/6: update [%s] with status [%s]...", accessory.displayName, value === 0 ? "opening" : "closing", );
         service
         ...
         if (!garageConfig.inched || garageConfig.inched === "false") {
            ...
            this.log("IGU5/6: not sending switch off command as inched through eWeLink...");
         }
         if (!sAccessory) {
            setTimeout(() => {
            ...
            this.log("IGU6/6: [%s] sensor defined so now will wait for the sensor to send update.", accessory.displayName);
            this.log("=== INTERNAL GARAGE UPDATE END ===");
            callback();
         }
      } catch (err) {
   ...
   internalFanUpdate(accessory, type, value, callback) {
      try {
         ...
         if (this.debug) {
            this.log("[%s] updating [%s] to [%s].", accessory.displayName, type, value);
         }
   ...
   externalLANUpdate(device) {
      try {
         if (this.debug) {
            this.log("LAN mode status update detected. This feature is in early stages.");
         }
      } catch (err) {
         this.log.warn("Generic Error Message - [%s]", err);
      }
   }
}
