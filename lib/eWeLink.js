...
class eWeLink {
   ...
   initialiseDevice(device) {
      ...
      if (!this.devicesInHB.has(device.deviceid + "SWX") && !this.devicesInHB.has(device.deviceid + "SW0")) {
         ...
         } else if (constants.devicesZBBridge.includes(device.uiid)) {
            
            device.params.subDevices.forEach(subDevice => { 
               this.addZBAccessory(device, subDevice);
            });      
   ...
   addZBAccessory(device, subDevice) {
      
      let hbDeviceId = subDevice.deviceid + "SWX";
      let channelCount = constants.chansFromUiid[subDevice.uiid],
         switchNumber = hbDeviceId.substr(-1).toString(),
         newDeviceName = subDevice.deviceid;

      let type = "";   

      if (this.customNameOverride.hasOwnProperty(subDevice.deviceid)) {
         newDeviceName = this.customNameOverride[subDevice.deviceid];
      }


      switch (subDevice.uiid) {
         case '1000':
            type = "switch";
            break;
         case '3026':
            ...
            break;
         default:
            this.log("[%s(%d)] could not be added.", newDeviceName, subDevice.uiid);
            return;
      }   
   ...
   externalSensorUpdate(accessory, params) {
      try {
         ...
         let service = accessory.getService(Service.ContactSensor),
            oldState = service.getCharacteristic(Characteristic.ContactSensorState).value,
