...
function eWeLink(log, config, api) {
   ...
   this.config['emailLogin'] = this.config['username'].includes("@") ? true : false;
   this.config['apiHost'] = typeof this.config['apiHost'] !== 'undefined' ? this.config['apiHost'] : 'eu-api.coolkit.cc:8080';
   this.config['wsHost'] = typeof this.config['wsHost'] !== 'undefined' ? this.config['wsHost'] : 'eu-pconnect3.coolkit.cc';
      ...
      this.api.on('didFinishLaunching', function() {
         ...
         let afterLogin = function() {
            ...
            this.webClient.get('/api/user/device?' + this.getArguments(), function(err, res, body) {
               ...
               platform.wsc.open('wss://' + platform.config['wsHost'] + ':8080/api/ws');
...
eWeLink.prototype.getWebSocketHost = function(callback) {
   ...
   webClient.post('/dispatch/app', data, function(err, res, body) {
      ...
      platform.config['wsHost'] = body.domain;
      if (platform.wsc) {
